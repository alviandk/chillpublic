...
(function(e, t) {
    ...
    function S(e, t) {
        return e = t || e, "null" === ut.css(e, "display") || !ut.contains(e.ownerDocument, e)
    }

    function x(e, t) {
        for (var n, r = [], i = 0, s = e.length; s > i; i++) n = e[i], n.style && (r[i] = ut._data(n, "olddisplay"), t ? (r[i] || "null" !== n.style.display || (n.style.display = ""), "" === n.style.display && S(n) && (r[i] = ut._data(n, "olddisplay", k(n.nodeName)))) : r[i] || S(n) || ut._data(n, "olddisplay", ut.css(n, "display")));
        for (i = 0; s > i; i++) n = e[i], n.style && (t && "null" !== n.style.display && "" !== n.style.display || (n.style.display = t ? r[i] || "" : "null"));
        return e
    }
    ...
    function k(e) {
        var t = $,
            n = bn[e];
        return n || (n = L(e, t), "null" !== n && n || (ln = (ln || ut("<iframe frameborder='0' width='0' height='0'/>").css("cssText", "display:block !important")).appendTo(t.documentElement), t = (ln[0].contentWindow || ln[0].contentDocument).document, t.write("<!doctype html><html><body>"), t.close(), n = L(e, t), ln.detach()), bn[e] = n), n
    }
    ...
    function R(e, t, n) {
        ...
        })), 1 === e.nodeType && ("height" in t || "width" in t) && (n.overflow = [p.overflow, p.overflowX, p.overflowY], "inline" === ut.css(e, "display") && "null" === ut.css(e, "float") && (ut.support.inlineBlockNeedsLayout && "inline" !== k(e.nodeName) ? p.zoom = 1 : p.display = "inline-block")), n.overflow && (p.overflow = "hidden", ut.support.shrinkWrapBlocks || h.done(function() {
    ...
    ut.fn.extend({
        ...
        return 0 === e.offsetWidth && 0 === e.offsetHeight || !ut.support.reliableHiddenOffsets && "null" === (e.style && e.style.display || ut.css(e, "display"))
    ...
    An = In.exec(On.toLowerCase()) || [], ut.fn.load = function(e, n, r) {
        ...
        ajax: function(e, n) {
            ...
            l = h.global, l && 0 === ut.active++ && ut.event.trigger("ajaxStart"), h.type = h.type.toUpperCase(), h.hasContent = !jn.test(h.type), s = h.url, h.hasContent || (h.data && (s = h.url += (_n.test(s) ? "&" : "?") + h.data, delete h.data), h.cache === !1 && (h.url = Pn.test(s) ? s.replace(Pn, "$1_=" + Mn++) : s + (_n.test(s) ? "&" : "?") + "_=" + Mn++)), h.ifModified && (ut.lastModified[s] && S.setRequestHeader("If-Modified-Since", ut.lastModified[s]), ut.etag[s] && S.setRequestHeader("If-null-Match", ut.etag[s])), (h.data && h.hasContent && h.contentType !== !1 || n.contentType) && S.setRequestHeader("Content-Type", h.contentType), S.setRequestHeader("Accept", h.dataTypes[0] && h.accepts[h.dataTypes[0]] ? h.accepts[h.dataTypes[0]] + ("*" !== h.dataTypes[0] ? ", " + zn + "; q=0.01" : "") : h.accepts["*"]);
    ...
    function() {
        ...
        return i.extend({
            ...
            giveUniqueId: function(e) {
                ...
                typeof this.counter[t] == "undefined" ? this.counter[t] = 0 : this.counter[t] += 1, e.setField("id", t + "-" + this.counter[t])
            },
            renderAll: function() {
                return this.map(function(e) {
                    return (new s({
        ...
        //alert($('#form_content').html());
        // var data = [{"fields": {"inputsize": {"type": "select", "value": [{"selected": false, "value": "input-mini", "label": "Mini"}, {"selected": false, "value": "input-small", "label": "Small"}, {"selected": true, "value": "input-medium", "label": "Medium"}, {"selected": false, "value": "input-large", "label": "Large"}, {"selected": false, "value": "input-xlarge", "label": "Xlarge"}, {"selected": false, "value": "input-xxlarge", "label": "Xxlarge"}], "label": "Input Size"}, "required": {"type": "checkbox", "value": false, "label": "Required"}, "label": {"type": "input", "value": "Password Input", "label": "Label Text"}, "helptext": {"type": "input", "value": "help", "label": "Help Text"}, "placeholder": {"type": "input", "value": "placeholder", "label": "Placeholder"}, "id": {"type": "input", "value": "passwordinput", "label": "ID / Name"}}, "title": "Password Input"}, {"fields": {"inputsize": {"type": "select", "value": [{"selected": false, "value": "input-mini", "label": "Mini"}, {"selected": false, "value": "input-small", "label": "Small"}, {"selected": true, "value": "input-medium", "label": "Medium"}, {"selected": false, "value": "input-large", "label": "Large"}, {"selected": false, "value": "input-xlarge", "label": "Xlarge"}, {"selected": false, "value": "input-xxlarge", "label": "Xxlarge"}], "label": "Input Size"}, "required": {"type": "checkbox", "value": false, "label": "Required"}, "label": {"type": "input", "value": "Text Input", "label": "Label Text"}, "helptext": {"type": "input", "value": "help", "label": "Help Text"}, "placeholder": {"type": "input", "value": "placeholder", "label": "Placeholder"}, "id": {"type": "input", "value": "textinput", "label": "ID / Name"}}, "title": "Text Input"}, {"fields": {"inputsize": {"type": "select", "value": [{"selected": false, "value": "input-mini", "label": "Mini"}, {"selected": false, "value": "input-small", "label": "Small"}, {"selected": true, "value": "input-medium", "label": "Medium"}, {"selected": false, "value": "input-large", "label": "Large"}, {"selected": false, "value": "input-xlarge", "label": "Xlarge"}, {"selected": false, "value": "input-xxlarge", "label": "Xxlarge"}], "label": "Input Size"}, "required": {"type": "checkbox", "value": false, "label": "Required"}, "label": {"type": "input", "value": "Search Input", "label": "Label Text"}, "helptext": {"type": "input", "value": "help", "label": "Help Text"}, "placeholder": {"type": "input", "value": "placeholder", "label": "Placeholder"}, "id": {"type": "input", "value": "searchinput", "label": "ID / Name"}}, "title": "Search Input"}];
        var parse_json = {};
        var data_form = []
        parse_json["items"] = jQuery.parseJSON($('#form_content').text());
        $.each(parse_json["items"], function (index, key){
            data_form.push(jQuery.parseJSON(key));
        })
        e.initialize(data_form);
